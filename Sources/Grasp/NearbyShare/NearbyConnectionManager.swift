...
struct EndpointInfo {
	...
	init?(data: Data) {
		...
		if hasName {
			...
			guard
				let _deviceName = String(data: data[18..<(18 + deviceNameLength)], encoding: .utf8)
			else { return nil }
	...
	OutboundNearbyConnectionDelegate
{
	...
	public var endpointID: [UInt8] = generateEndpointID()
	...
	private func initMDNS() {
		...
		let configuredName = Host.current().localizedName ?? "MacBook Pro"
		let endpointInfo = EndpointInfo(name: configuredName, deviceType: .computer)
	...
	public func stopMDNS() {
		mdnsService?.stop()
		mdnsService = nil
	}

	public func restartMDNS() {
		stopMDNS()
		endpointID = NearbyConnectionManager.generateEndpointID()
	...
	private func endpointID(for service: NWBrowser.Result) -> String? {
		guard
			case NWEndpoint.service(name: let serviceName, type: _, domain: _, interface: _) =
				service.endpoint
		else { return nil }
	...
	private func maybeAddFoundDevice(service: NWBrowser.Result) {
		...
		guard case NWBrowser.Result.Metadata.bonjour(let txtRecord) = service.metadata else {
			return
		}
		...
		if endpointInfo.name != nil {
			deviceInfo = addFoundDevice(
				foundService: &foundService, endpointInfo: endpointInfo, endpointID: endpointID)
		}

		if let qrData = endpointInfo.qrCodeData, qrCodeAdvertisingToken != nil {
			#if DEBUG
				print(
					"Device has QR data: \(qrData.base64EncodedString()), our advertising token is \(qrCodeAdvertisingToken!.base64EncodedString())"
				)
			#endif
			if qrData == qrCodeAdvertisingToken! {
				...
				do {
					...
					let decryptedName = try AES.GCM.open(
						box, using: qrCodeNameEncryptionKey!,
						authenticating: qrCodeAdvertisingToken!)
					guard let name = String.init(data: decryptedName, encoding: .utf8) else {
						return
					}
					endpointInfo.name = name
					let deviceInfo = addFoundDevice(
						foundService: &foundService, endpointInfo: endpointInfo,
	...
	private func addFoundDevice(
		foundService: inout FoundServiceInfo, endpointInfo: EndpointInfo, endpointID: String
	) -> RemoteDeviceInfo {
	...
	private func maybeRemoveFoundDevice(service: NWBrowser.Result) {
		guard let endpointID = endpointID(for: service) else { return }
		guard foundServices.removeValue(forKey: endpointID) != nil else { return }
		for delegate in shareExtensionDelegates {
	...
	public func generateQrCodeKey() -> String {
		...
		qrCodeAdvertisingToken = NearbyConnection.hkdf(
			inputKeyMaterial: ikm, salt: Data(), info: "advertisingContext".data(using: .utf8)!,
			outputByteCount: 16
		).data()
		qrCodeNameEncryptionKey = NearbyConnection.hkdf(
			inputKeyMaterial: ikm, salt: Data(), info: "encryptionKey".data(using: .utf8)!,
			outputByteCount: 16)
		qrCodeData = keyData
	...
	public func startOutgoingTransfer(
		deviceID: String, delegate: ShareExtensionDelegate, urls: [URL]
	) {
		guard let info = foundServices[deviceID] else { return }
		let tcp = NWProtocolTCP.Options.init()
		tcp.noDelay = true
		let nwconn = NWConnection(
			to: info.service.endpoint, using: NWParameters(tls: .none, tcp: tcp))
		...
		let transfer = OutgoingTransferInfo(
			service: info.service, device: info.device!, connection: conn, delegate: delegate)
